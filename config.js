/*
 * config.js  —  The single source of truth for your ordination page.
 *
 * HOW TO CUSTOMIZE
 * ─────────────────
 * The easiest way to customize this page is via the settings panel:
 *   1. Open your live site
 *   2. Click the ⚙ gear icon in the bottom-right corner
 *   3. Edit your details and click "Save to GitHub"
 *
 * You can also edit this file directly in GitHub's web editor (click the
 * pencil icon on this file). Change the values between the quotation marks,
 * then click "Commit changes". Your site will update within a minute or two.
 *
 * IMPORTANT: Do not change the structure of this file — only change the
 * values inside the quotation marks (or true/false for on/off settings).
 */

window.CONFIG = {

  // ── Identity ─────────────────────────────────────────────────────────────
  // The main descriptive text at the top of the page.
  identity: {
    candidateName: "Gabriel Whitlow",
    eventTitle:    "Diaconate Ordination — 2026",
    eyebrow:       "Archdiocese of Atlanta",
    leadText:      "With gratitude for the Lord's call and the support of family and friends, I look toward ordination to the transitional diaconate. Please keep me in your prayers as the day approaches.",
  },

  // ── Appearance ───────────────────────────────────────────────────────────
  // Theme options: "burgundy"  "navy"  "forest"  "slate"
  appearance: {
    theme: "burgundy",
  },

  // ── Section visibility ────────────────────────────────────────────────────
  // Set any section to false to hide it from the page.
  sections: {
    countdown:        true,
    hero:             true,
    details:          true,
    intentions:       true,
    catechesis:       true,
    livestream:       true,
    rsvp:             true,
    ordinationMass:   true,
    thanksgivingMass: true,
  },

  // ── Countdown timer ───────────────────────────────────────────────────────
  // targetDate format: "YYYY-MM-DDTHH:MM:SS-HH:MM"  (ISO 8601 with timezone offset)
  // Example: "2026-05-23T10:00:00-04:00" = May 23 2026, 10 AM Eastern
  countdown: {
    targetDate:     "2026-05-23T10:00:00-04:00",
    label:          "Countdown to Ordination",
    reflectionText: "I was not a fan of having a countdown timer until someone flipped the interpretation on its head for me. They saw it as the number of Masses, Holy Hours, and Rosaries left to pray beforehand. That insight convinced me to add this here.",
  },

  // ── Event details card ───────────────────────────────────────────────────
  details: {
    heading:     "Ordination Day",
    dateDisplay: "May 23, 2026",
    body:        "The Archdiocese of Atlanta has confirmed the date and location. I look forward to celebrating this day with all of you — whether in person or in spirit.",
  },

  // ── Prayer intentions ────────────────────────────────────────────────────
  intentions: {
    heading: "Prayer Intentions",
    items: [
      "Perseverance in formation and fidelity to the Gospel",
      "Deepening charity in service to the people of God",
      "Gratitude for everyone who has accompanied me along the way",
    ],
  },

  // ── Catechesis section ───────────────────────────────────────────────────
  catechesis: {
    eyebrow:    "Catechesis",
    heading:    "What Is a Diaconate Ordination?",
    paragraphs: [
      "The transitional diaconate is the final step before priesthood. A candidate promises obedience, embraces a life ordered toward service, and is configured to Christ the Servant to proclaim the Gospel, preach, and assist at the altar. Receiving the Book of the Gospels and putting on the dalmatic signify this mission of charity, word, and liturgy in a visible way.",
      "During the rite the bishop prays the Prayer of Ordination, lays hands upon the candidate, and invokes the Holy Spirit. The newly ordained deacon then serves at the altar and dismisses the assembly with the mandate to bring the Good News into the world.",
    ],
  },

  // ── Livestream ────────────────────────────────────────────────────────────
  // Leave url empty ("") to show the "Coming Soon" message.
  // When you have a link, paste it here: "https://youtube.com/live/..."
  livestream: {
    heading:       "Watch Live",
    url:           "",
    comingSoonText: "Coming Soon — check back closer to the date.",
  },

  // ── RSVP / Stay in Touch (Google Form embed) ─────────────────────────────
  // To get your Google Form embed URL:
  //   1. Open your form in Google Forms
  //   2. Click Send → Embed tab (<>)
  //   3. Copy just the src="..." URL from the iframe code
  rsvp: {
    heading:      "Stay in Touch",
    introText:    "If you would like to receive updates or share how you are praying, please leave your contact information below. I'll keep the list focused on ordination news.",
    formEmbedUrl: "https://docs.google.com/forms/d/e/1FAIpQLSfoTlYYZnjfvp86DeAFMD34wMsWuDFxu2eboKEq46SV4joajQ/viewform?embedded=true",
  },

  // ── Mass of Ordination ───────────────────────────────────────────────────
  // mapsEmbedUrl: from Google Maps → Share → Embed a map → copy the src URL
  ordinationMass: {
    eyebrow:      "Mass of Ordination",
    heading:      "Diaconate Ordination",
    description:  "The final details from the Archdiocese will be shared here, along with arrival guidance and parking notes.",
    parish:       "St. Peter Chanel Catholic Church",
    date:         "May 23, 2026",
    time:         "10:00 AM Eastern",
    address:      "11330 Woodstock Rd, Roswell, GA 30075",
    parking:      "On-campus parking available by the Church and Schools.",
    mapsEmbedUrl: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d5010.256663636794!2d-84.38988440156336!3d34.055765091397916!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x88f573111d85d80f%3A0x6bef3af4908a9b07!2sSt.%20Peter%20Chanel%20Catholic%20Church!5e0!3m2!1sen!2sus!4v1762803330253!5m2!1sen!2sus",
  },

  // ── Mass of Thanksgiving ─────────────────────────────────────────────────
  thanksgivingMass: {
    eyebrow:      "Mass of Thanksgiving",
    heading:      "Celebration Mass & Reception",
    description:  "All are welcome to pray together the day after ordination.",
    parish:       "St. Peter's Catholic Church",
    date:         "May 24, 2026",
    time:         "11:00 AM Eastern",
    address:      "200 Lafayette Pkwy, LaGrange, GA 30241",
    parking:      "Light refreshments are planned immediately after Mass.",
    mapsEmbedUrl: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3344.705499647885!2d-85.03162428833699!3d33.03788847344538!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x888b569b7b4a1595%3A0xa0a88297a7c80116!2sSt.%20Peter's%20Catholic%20Church!5e0!3m2!1sen!2sus!4v1762803696059!5m2!1sen!2sus",
  },

  // ── Extra / stock elements ────────────────────────────────────────────────
  // These are optional blocks added via the "Add Elements" tab in the
  // settings panel. They appear at the bottom of the page in the order
  // listed here. You can also add them manually by following the examples:
  //
  // Scripture / Quote:
  //   { type: "scripture", reference: "John 13:35", text: "By this all will know that you are my disciples..." }
  //
  // Custom text section:
  //   { type: "customText", heading: "A Note from the Family", body: "We are so grateful..." }
  //
  // Prayer partners list:
  //   { type: "prayerPartners", heading: "Prayer Partners", names: ["Alice Smith", "Bob Jones"] }
  //
  // Second venue card (same fields as ordinationMass above):
  //   { type: "venue", eyebrow: "Reception", heading: "...", description: "...", parish: "...",
  //     date: "...", time: "...", address: "...", parking: "...", mapsEmbedUrl: "..." }
  //
  // YouTube video:
  //   { type: "youtubeEmbed", heading: "About the Diaconate", videoId: "XXXXXXXXXXX" }
  //   (The videoId is the part after "?v=" in a YouTube URL)
  //
  extraElements: [],

};
